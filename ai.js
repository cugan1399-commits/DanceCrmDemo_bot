...
async function toolCheckProductCatalog({ search_query, chatId, db, username }) {
  try {
    ...
    const photoChecks = await Promise.all(
      list.slice(0, 8).map(p => getProductPhotoUrl(p.id).catch(() => null))
    );
