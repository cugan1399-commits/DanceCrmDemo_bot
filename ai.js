...
import { bitrixCall, getDeal, addDealComment, createInterestDeal, createOrderDeal, convertDealToOrder, getProductPhotoUrl, ORG_TIMEZONE } from './bitrix.js';
...
export const AI_TOOLS = [
  {
    ...
    function: {
      ...
      description:
        'Поиск обуви в коммерческом каталоге Битрикс24, проверка цен, описания и доступного остатка. Обязательно вызывай этот инструмент, если клиент в Telegram спрашивает, что есть в наличии, сколько стоит конкретная модель или какие кроссовки доступны. Если по запросу находится ровно одна конкретная модель, её фото отправляется клиенту автоматически вместе с ответом этого инструмента — отдельно вызывать send_product_photo сразу после этого не нужно.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Отправить клиенту в Telegram настоящее фото товара из каталога Битрикс24. Фото уже уходит автоматически при первом упоминании конкретной модели (см. check_product_catalog) — этот инструмент вызывай только для ПОВТОРНОЙ просьбы прислать фото ещё раз. Это НЕ ответ текстом — фото уйдёт отдельным сообщением автоматически, тебе не нужно ничего вставлять в текст самому.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'create_order',
      description:
        'Оформить реальный заказ клиента в CRM. Вызывай ЭТОТ инструмент ТОЛЬКО когда выполнены ДВА условия: 1) у тебя есть ВСЕ пять полей, явно подтверждённые клиентом в переписке (точное название товара, сверенное через check_product_catalog, размер, цвет, адрес доставки и способ оплаты); 2) ты уже показал клиенту сводку этих пяти пунктов ОДНИМ отдельным сообщением и явно спросил "всё верно, оформляем?" (или похоже), и клиент на это ответил согласием (например "да"/"верно"/"оформляй"). Если клиент вместо согласия что-то поправил — учти правку, покажи обновлённую сводку и снова дождись подтверждения, не вызывай инструмент сразу. Если чего-то из пяти полей не хватает — НЕ вызывай инструмент и не показывай сводку, а сначала спроси недостающее текстом (по одному пункту за раз). До успешного вызова этого инструмента НИКОГДА не говори клиенту фразы вроде "заказ оформлен"/"уже оформляю"/"ваш заказ принят" — это будет неправдой, потому что реального заказа в системе ещё нет.',
      parameters: {
        type: 'object',
        properties: {
...
async function toolCheckProductCatalog({ search_query, chatId, db, username }) {
  try {
    ...
    // промпте её не останавливал — модель слабая). Теперь наличие фото —
    // РЕАЛЬНО проверенный факт прямо в результате инструмента: модели остаётся
    // только процитировать его, а не придумывать.
    // На этом этапе list.length === 1 (случай >1 уже обработан выше и вернул
    // ответ раньше) — то есть клиент назвал/уточнил РОВНО одну конкретную
    // модель. Поэтому вместо того чтобы просто пометить "фото есть" и ждать,
    // пока клиент попросит его отдельно, ищем реальный маркер фото и клеим его
    // в результат — фото уйдёт клиенту в этом же ходе, автоматически.
    const photoMarkers = await Promise.all(
      list.slice(0, 8).map(p => resolveProductPhotoMarker(db, p.id, p.name).catch(() => null))
    );
    const lines = list.slice(0, 8).map((p, i) => {
      ...
      const photoStatus = photoMarkers[i] ? 'фото есть' : 'фото нет';
      return `• ${p.name}: ${price}${stock}${sizesInfo}, ${photoStatus}${desc}`;
    });
    // Берём первый реально найденный маркер (в норме он один, т.к. карточка одна) —
    // именно он и уйдёт клиенту отдельным сообщением через общий парсинг
    // [[SEND_PHOTO...]] в handleUserMessage, точно так же, как для send_product_photo.
    const photoMarker = photoMarkers.find(Boolean) || null;
    const photoInstruction = photoMarker
      ? `${photoMarker} Фото товара уже отправлено клиенту отдельным сообщением ПРЯМО СЕЙЧАС — не спрашивай, хочет ли он посмотреть фото, оно уже ушло, просто упомяни в ответе, что прислал фото.\n`
      : '';
    const resultText = `${photoInstruction}Найдено в каталоге:\n${lines.join('\n')}`;
    await logProductInterestToDeal({ chatId, db, username, search_query, summary: resultText });
    return resultText;
  } catch (err) {
...
// Общий поиск фото по ОДНОЙ карточке товара — вынесено из toolSendProductPhoto,
// чтобы этой же логикой (файл от менеджера -> локальный файл -> Битрикс) мог
// пользоваться и toolCheckProductCatalog для автоматической отправки фото сразу
// при выборе конкретной модели (см. ФИКС ниже), не дублируя три способа поиска.
// Возвращает готовый служебный маркер ([[SEND_PHOTO_FILEID:...]] или
// [[SEND_PHOTO:...]]) или null, если фото у этой карточки не нашлось никаким способом.
async function resolveProductPhotoMarker(db, productId, productName) {
  const fileId = await findProductPhotoFileId(db, productName);
  if (fileId) return `[[SEND_PHOTO_FILEID:${fileId}]]`;
  const localUrl = findLocalProductPhotoUrl(productName);
  if (localUrl) return `[[SEND_PHOTO:${localUrl}]]`;
  const bitrixUrl = await getProductPhotoUrl(productId).catch(() => null);
  if (bitrixUrl) return `[[SEND_PHOTO:${bitrixUrl}]]`;
  return null;
}
...
async function toolSendProductPhoto({ search_query, product_name, chatId, db, username }) {
  ...
  try {
    ...
    for (const product of list.slice(0, 15)) {
      const marker = await resolveProductPhotoMarker(db, product.id, product.name);
      if (marker) {
        return `${marker} Фото товара "${product.name}" отправлено клиенту отдельным сообщением.`;
      }
    }
...
async function toolCreateOrder({ product_name, size, color, delivery_address, payment_method, chatId, db, username }) {
  try {
    ...
    // Если у этого чата уже есть открытая сделка "Интерес к товару" (клиент
    // спрашивал про товар до того, как решил заказать) — превращаем ЕЁ в заказ,
    // а не создаём вторую параллельную сделку на того же клиента.
    let dealId = await getActiveDealIfOpen({ db, chatId, collection: INTEREST_DEAL_COLLECTION }).catch(err => {
      console.warn('Не удалось проверить активную сделку интереса к товару перед оформлением заказа:', err.message);
      return null;
    });
    if (dealId) {
      await convertDealToOrder(dealId, {
        chatId, username, productName: product_name, size, color,
        deliveryAddress: delivery_address, paymentMethod: payment_method,
      });
    } else {
      dealId = await createOrderDeal({
    ...
    // ФИКС "клиенту в чат ушло 'сделка #51 создана в CRM'": слабая модель раньше
    // дословно копировала эту фразу в ответ клиенту. Внутренний номер сделки и
    // слово "CRM" явно помечаем как служебное — не для клиента.
    return `Заказ успешно оформлен (внутренний номер #${dealId} — это только для тебя, клиенту его НЕ называй и слово "CRM"/"сделка" ему не говори).${payButtonMarker} Подтверди клиенту простыми словами, что заказ принят${payment_method === 'online' ? ', и что кнопка для оплаты придёт следующим сообщением' : ''} — без внутренних деталей.`;
  } catch (err) {
...
Ты умеешь:
— отвечать на вопросы о товарах/услугах и ценах (используй check_product_catalog) — вызывай его при ЛЮБОМ упоминании товара клиентом, даже если он не спросил явно цену/наличие (например "мне нужны кроссовки" — это тоже повод вызвать check_product_catalog, а не отвечать по памяти). НИКОГДА не упоминай модель/товар, которого не было в результатах check_product_catalog В ЭТОМ диалоге — не предлагай клиенту "другие модели" или "модели с фото", которые ты не проверял через инструмент прямо сейчас. Про наличие фото у товара говори СТРОГО то, что указано в результате check_product_catalog ("фото есть"/"фото нет") — это реально проверенный факт, никогда не переопределяй и не додумывай его от себя. ВАЖНО про объём ответа: если клиент назвал только общую категорию, а не конкретную модель ("хочу кроссовки", "что у вас есть из обуви") — в ответе перечисли ТОЛЬКО названия моделей/брендов из результата инструмента, коротко, без цен, размеров, остатков и описаний, и спроси, какая модель интересует. Полную информацию (цену, остаток, размеры, описание) сообщай только когда клиент уже назвал конкретную модель или прямо спросил про цену/наличие/размер;
...
— отправлять фото товара клиенту (используй send_product_photo) по ПОВТОРНОЙ просьбе показать/прислать фото ещё раз. Фото при ПЕРВОМ упоминании конкретной модели уже уходит клиенту АВТОМАТИЧЕСКИ вместе с результатом check_product_catalog (см. инструкцию в результате инструмента) — в этом случае просто упомяни в ответе, что прислал фото, не спрашивай "хотите посмотреть фото?" и не вызывай send_product_photo второй раз. НИКОГДА не утверждай, что у товара "нет фото в каталоге" или наоборот "фото есть", кроме как процитировав то, что реально написано в результате check_product_catalog/send_product_photo — не додумывай это от себя;
— оформлять заказ (используй create_order), но ТОЛЬКО после того, как по очереди уточнишь у клиента: 1) какой именно товар (сверь через check_product_catalog), 2) размер, 3) цвет, 4) адрес доставки, 5) способ оплаты. Про способ оплаты спрашивай именно ВЫБОР, а не проси "оплатить" (платить пока нечего — заказа ещё нет): предложи РОВНО два варианта и используй примерно такую формулировку — "Как вам удобнее оплатить: онлайн заранее или курьеру при получении?" (это напрямую влияет на то, в какую стадию воронки попадёт сделка, поэтому не додумывай сам, если клиент ответил расплывчато — переспроси). Спрашивай недостающее по одному пункту за раз, не вываливай все вопросы сразу и не пропускай пункты (нельзя переходить к следующему шагу или тем более к сводке, пока текущий пункт не получен явно от клиента). ВАЖНО: даже если адрес или способ оплаты уже упоминались РАНЬШЕ в этой переписке (например, в прошлом заказе или в другом разговоре) — для НОВОГО заказа их всё равно нужно спросить заново текстом и получить свежий явный ответ именно сейчас; никогда не подставляй их молча из истории, даже если "и так понятно". Когда все пять пунктов собраны заново в этом заказе — ПЕРЕД вызовом create_order покажи клиенту сводку одним сообщением (товар, размер, цвет, адрес, способ оплаты) и спроси "всё верно, оформляем?", и вызывай инструмент только после того, как клиент подтвердил согласием. Пока все пять пунктов не собраны и клиент не подтвердил сводку — НЕ вызывай create_order и НЕ говори клиенту ничего похожего на то, что заказ оформлен, принят или уже готовится: это будет ложью, пока сделка реально не создана инструментом. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать и называть клиенту номер заказа/сделки, дату доставки или статус оплаты ("оплата подтверждена") — эти факты существуют только после реального успешного вызова create_order, и даже тогда номер сделки клиенту не называется (см. результат инструмента). Клиенту НИКОГДА не упоминай слова "CRM", "сделка", "база", номера сделок, и не описывай свои внутренние шаги/планы ("далее я сделаю...", "запишем в систему" и т.п.) — для клиента это просто "ваш заказ", без кухни;
...
// ФИКС "любое упоминание слова 'фото' в сообщении вызывало повторную отправку
// фото и глушило остальную часть сообщения": старый PHOTO_INTENT_RE ловил
// голую подстроку "фот" ГДЕ УГОДНО в тексте — например "цвет белый КАК НА
// ФОТО, 43 размер есть, как мне их приобрести?" тоже совпадал, хотя клиент не
// просил фото, а спрашивал, как оформить заказ. Из-за того что этот шорткат
// отрабатывает ДО модели и просто возвращает фото, весь остальной смысл
// сообщения (вопрос "как приобрести") терялся — модель его вообще не видела.
// Теперь: 1) регэксп требует явного глагола-просьбы рядом со словом "фото"
// (а не голого упоминания), и 2) шорткат не срабатывает, если в сообщении
// есть другие явные признаки содержательного вопроса/шага оформления заказа —
// такие сообщения всегда должны уходить в модель целиком, а не обрезаться.
const PHOTO_REQUEST_RE = /(пришли|пришлите|скинь|скиньте|отправь|отправьте|покажи|покажите|дай|дайте|вышли|вышлите|можно\s+(увидеть|посмотреть))\s*(ещё\s+раз\s+)?(фото|картинк\w*|снимок)|^\s*(фото|картинк\w*|снимок)\s*[?!.]*\s*$|как\s*(он[аи]?\s*)?выгляд/i;
const OTHER_INTENT_RE = /приобрес|купит|заказ|оформ|размер|цвет|адрес|оплат|доставк|сколько\s*сто|цена/i;

async function tryPhotoShortcut({ db, chatId, userText, username }) {
  if (!PHOTO_REQUEST_RE.test(userText)) return null;
  if (OTHER_INTENT_RE.test(userText)) return null; // сообщение явно про другое — пусть разбирается модель целиком
...
export async function handleUserMessage({ db, chatId, userText, username }) {
  ...
  // ФИКС "Создал заказ №12345 с доставкой к 2024-04-15... оплата подтверждена" —
  // прежний вариант регэкспа ловил только порядок слов "заказ + оформлен/создан",
  // но не ловил обратный порядок "создал + заказ", придуманный номер сделки,
  // придуманную дату доставки и придуманное "оплата подтверждена" — то есть ровно
  // те формулировки, которыми модель целиком сочинила несуществующий заказ, ни
  // разу не вызвав create_order. Теперь проверяем более широкий набор явных
  // признаков придуманного заказа — если сработал ХОТЯ БЫ ОДИН, а create_order
  // в этом ходе реально не отработал успешно, сообщение блокируется.
  const ORDER_SUCCESS_CLAIM_RE = new RegExp([
    'заказ\\s+(успешно\\s+)?(создан|оформлен|принят)',       // "заказ оформлен"
    '(создал|оформил|принял)[а]?\\s+(ваш\\s+)?заказ',        // "создал заказ", "оформил ваш заказ"
    'заказ\\s*№\\s*\\d+',                                     // "заказ №12345"
    '№\\s*\\d+.{0,20}(заказ|доставк)',                        // "№12345 с доставкой..."
    'оплата.{0,15}(подтвержд|прош[её]л|получен)',             // "оплата онлайн подтверждена"
    '\\bдоставк\\w*\\s+к\\s+\\d',                             // "доставкой к 2024-04-15"
  ].join('|'), 'i');
